...
MIN_RUSAGE_MEM=${3:-"3072"}
MIN_DISCRIMINANT_LCSS_JOBNAME_LENGTH=${4:-"3"}
LOWER_DATE=${5-$(date '+%Y-%m-%d' -d'-10 days')} #30 days')}
UPPER_DATE=${6-$(date '+%Y-%m-%d')}
ACCT_SERVER=${7:-"127.0.0.1"}
ACCT_USER=${8:-"root"}
ACCT_PASSWORD="root"
...
./job_mem_thresh $MEM_LOWER_LIMIT $MEM_UPPER_LIMIT $MIN_RUSAGE_MEM $MIN_DISCRIMINANT_LCSS_JOBNAME_LENGTH "$LOWER_DATE" "$UPPER_DATE" "$ACCT_SERVER" "$ACCT_USER" "$ACCT_PASSWORD" "$ACCT_DATABASE" "$MAIL_CMD" "$MAIL_FROM" "$MAIL_TO"
